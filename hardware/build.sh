...
# rm -rf ./outputs/outlines
rm -rf ./outputs/pcbs
# rm -rf ./outputs/points
...
if [ ! -e freerouting/freerouting-SNAPSHOT.jar ]; then
    curl https://github.com/freerouting/freerouting/releases/dmwnload/SNAPSHOT/freerouting-SNAPSHOT-20241111_140100.jar -L -o freerouting/freerouting-SNAPSHOT.jar
fi

for plate in ${plates}
do
    echo "\n\n>>>>>> Processing $plate <<<<<<\n\n"
    ${container_cmd} run ${container_args} ${kicad_auto_image} kibot -b outputs/pcbs/${plate}.kicad_pcb -c kibot/default.kibot.yaml
done
